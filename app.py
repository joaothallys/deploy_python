...
def upload_file():
    try:
        ...
        try:
            # Especificar o tipo de conteúdo do arquivo
        ...
        if connection:
            try:
                ...
                cursor.execute("INSERT INTO files (filename, file_url, content_type) VALUES (%s, %s, %s)",
                               (filename, file_url, content_type))
                connection.commit()
            except Error as e:
    ...
    try:
        ...
        if connection:
            try:
                cursor = connection.cursor()
                cursor.execute(
                    "SELECT file_url, content_type FROM files WHERE filename = %s", (filename,))
                row = cursor.fetchone()
                if row:
                    file_url = row[0]
                    content_type = row[1]
                    return jsonify(fileUrl=file_url, type=content_type), 200
                else:
                    print(
                        f"File not found in database for filename: {filename}")
